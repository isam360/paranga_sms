...
class Command(BaseCommand):
    help = "Create subjects and teachers from predefined data"
    ...
    def handle(self, *args, **kwargs):
        ...
        subjects = [
            ...
            {"name": "Bookkeeping", "code": "BKEEP"},
            {"name": "Commerce", "code": "COMM"},
            {"name": "Agriculture", "code": "AGRI"},
            {"name": "Religion", "code": "REL"},
            {"name": "Debate", "code": "DEB"},
            {"name": "Sports and Games", "code": "SPG"},
            {"name": "Scouts & Clubs", "code": "SCL"},
        ]
        ...
        for data in teacher_data:
            ...
            if data["assigned_class"]:
                assigned_class, _ = SchoolClass.objects.get_or_create(
                    name=data["assigned_class"]
                )
            ...
            # Assign subjects
            subject_names = data["subject_specialization"]
            subjects = Subject.objects.filter(name__in=subject_names)
            teacher.subject_specialization.set(subjects)

            self.stdout.write(self.style.SUCCESS(
                f"{'Created' if created else 'Updated'}: {teacher.full_name}"
            ))

        self.stdout.write(self.style.SUCCESS("🎉 All teachers created successfully!"))
